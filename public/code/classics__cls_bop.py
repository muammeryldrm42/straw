...
def balanceOfPower(c):
    if len(c) < 30:
        return make_signal(reason="Insufficient data")
    i = len(c) - 1
    a = atr(c, 14)
    bop = sma(bopArr(c), 14)
    if bop[i] > 0  and  bop[i - 1] <= 0:
        return mk(c, i, "long", a, 0.68, "Balance of Power turned positive (buyers in control)")
    if bop[i] < 0  and  bop[i - 1] >= 0:
        return mk(c, i, "short", a, 0.68, "Balance of Power turned negative (sellers in control)")
    return make_signal(reason="No BOP zero-cross")
