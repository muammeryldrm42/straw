...
def relativeVigor(c):
    ...
    rvi = _d['rvi']
    sig = _d['sig']
    if rvi[i] > sig[i]  and  rvi[i - 1] <= sig[i - 1]:
        return mk(c, i, "long", a, 0.69, "RVI crossed above its signal line")
    if rvi[i] < sig[i]  and  rvi[i - 1] >= sig[i - 1]:
        return mk(c, i, "short", a, 0.69, "RVI crossed below its signal line")
    return make_signal(reason="No RVI cross")
