...
def lastTwoLows(c, end, lb=3, span=40):
    ...
    def lastTwoHighs(c, end, lb=3, span=40):
        ...
        def multiDivergence(c):
            if len(c) < 50:
                return make_signal(reason="Insufficient data")
            i = len(c) - 1
            a = atr(c, 14)
            r = detectDiv(c, rsi([x['close'] for x in c], 14), i, a, "RSI", 0.7)
            m = detectDiv(c, macd([x['close'] for x in c])['histogram'], i, a, "MACD", 0.7)
            if r['signal'] == "long"  and  m['signal'] == "long":
                return mk(c, i, "long", a, 0.76, "Multi-divergence: RSI + MACD both bullish")
            if r['signal'] == "short"  and  m['signal'] == "short":
                return mk(c, i, "short", a, 0.76, "Multi-divergence: RSI + MACD both bearish")
            return make_signal(reason="No multi-oscillator divergence")
