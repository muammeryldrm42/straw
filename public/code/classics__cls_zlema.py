...
def zlemaCross(c):
    ...
    fast = zlema(closes, 9)
    slow = zlema(closes, 21)
    if fast[i] > slow[i]  and  fast[i - 1] <= slow[i - 1]:
        return mk(c, i, "long", a, 0.7, "Zero-Lag EMA bullish cross")
    if fast[i] < slow[i]  and  fast[i - 1] >= slow[i - 1]:
        return mk(c, i, "short", a, 0.7, "Zero-Lag EMA bearish cross")
    return make_signal(reason="No ZLEMA cross")
