...
def signOfWeakness(c):
    ...
    if c[i]['close'] < lo  and  c[i]['close'] < c[i]['open']  and  (c[i]['open'] - c[i]['close']) > a[i]  and  c[i]['volume'] > avgV * 1.5:
        return mk(c, i, "short", lo + a[i], [c[i]['close'] - a[i] * 2, c[i]['close'] - a[i] * 4], 0.72, "Sign of Weakness (range breakdown + volume)")
    return make_signal(reason="No SOW")
