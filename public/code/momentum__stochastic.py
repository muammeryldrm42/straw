...
def stochastic(c):
    ...
    if k[i] < 20  and  k[i - 1] <= d[i - 1]  and  k[i] > d[i]:
        return mk(c, i, "long", a, 0.71, f"Stochastic bullish cross @ oversold ({k[i]})")
    if k[i] > 80  and  k[i - 1] >= d[i - 1]  and  k[i] < d[i]:
        return mk(c, i, "short", a, 0.71, f"Stochastic bearish cross @ overbought ({k[i]})")
    return make_signal(reason=f"Stochastic {k[i]}")
