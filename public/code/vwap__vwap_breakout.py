...
def vwapBreakout(c):
    ...
    vwap = _d['vwap']
    prev = rollingVwap(c, i - 1, 30)
    if c[i - 1]['close'] <= prev.vwap  and  c[i]['close'] > vwap:
        return mk(c, i, "long", a, 0.7, "Reclaimed VWAP (bullish)")
    if c[i - 1]['close'] >= prev.vwap  and  c[i]['close'] < vwap:
        return mk(c, i, "short", a, 0.7, "Lost VWAP (bearish)")
    return make_signal(reason="No VWAP cross")
