...
def qstick(c):
    ...
    oc = [x['close'] - x['open'] for x in c]
    q = sma(oc, 14)
    i = len(c) - 1
    a = atr(c, 14)
    if q[i - 1] <= 0  and  q[i] > 0:
        return mk(c, i, "long", a, 0.68, "Qstick turned positive (buying pressure)")
    if q[i - 1] >= 0  and  q[i] < 0:
        return mk(c, i, "short", a, 0.68, "Qstick turned negative (selling pressure)")
    return make_signal(reason=f"Qstick {q[i]}")
