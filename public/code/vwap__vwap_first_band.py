...
def vwapFirstBand(c):
    ...
    vwap = _d['vwap']
    sd = _d['sd']
    if c[i - 1]['close'] < vwap  and  c[i]['close'] > vwap + sd:
        return mk(c, i, "long", a, 0.68, "Pushed through VWAP +1σ (momentum)")
    if c[i - 1]['close'] > vwap  and  c[i]['close'] < vwap - sd:
        return mk(c, i, "short", a, 0.68, "Pushed through VWAP -1σ (momentum)")
    return make_signal(reason="Within first VWAP band")
