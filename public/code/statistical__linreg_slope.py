...
def linRegSlope(c):
    ...
    i = len(c) - 1
    a = atr(c, 14)
    s = linRegFull(closes, i, p).slope
    sPrev = linRegFull(closes, i - 1, p).slope
    if sPrev <= 0  and  s > 0:
        return mkS(c, i, "long", a, 0.69, "Linear regression slope turned up")
    if sPrev >= 0  and  s < 0:
        return mkS(c, i, "short", a, 0.69, "Linear regression slope turned down")
