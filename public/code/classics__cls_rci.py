...
def rciCross(c):
    ...
    now = rci(c, i, 9)
    prev = rci(c, i - 1, 9)
    if now > -80  and  prev <= -80:
        return mk(c, i, "long", a, 0.68, "RCI turning up from oversold (-80)")
    if now < 80  and  prev >= 80:
        return mk(c, i, "short", a, 0.68, "RCI turning down from overbought (+80)")
    return make_signal(reason="RCI mid-range")
