...
def nr7(c):
    ...
    prev = c[i - 1]
    last7 = c[i - 7: i]
    isNR7 = rng(prev) == min(last7.__map__(rng))
    if isNR7  and  c[i]['close'] > prev['high']:
        return mkPA(c, i, "long", prev['low'] - 0.3 * a[i], 0.71, "NR7 breakout up (strong contraction)")
    if isNR7  and  c[i]['close'] < prev['low']:
        return mkPA(c, i, "short", prev['high'] + 0.3 * a[i], 0.71, "NR7 breakdown (strong contraction)")
    return make_signal(reason="No NR7 break")
