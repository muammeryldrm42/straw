...
def bbRsiStoch(c):
    ...
    i = len(c) - 1
    a = atr(c, 14)
    if closes[i] <= bb['lower'][i]  and  r[i] < 35  and  k[i] < 25:
        return mkH(c, i, "long", a, 0.75, "Triple oversold: BB lower + RSI + Stochastic")
    if closes[i] >= bb['upper'][i]  and  r[i] > 65  and  k[i] > 75:
        return mkH(c, i, "short", a, 0.75, "Triple overbought: BB upper + RSI + Stochastic")
    return make_signal(reason="No triple-oscillator confluence")
