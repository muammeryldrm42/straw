...
def gannFan(c):
    ...
    for k in range(0, len(w)):
        ...
        if w[k]['high'] > w[hiIdx]['high']:
            hiIdx = k
    unit = a[i]
    # 1x1 açı = bar başına 1 birim (ATR)
    if hiIdx < loIdx:
        # dip daha yeni → yukarı 1x1
        bars = len(w) - 1 - loIdx
        gann = w[loIdx]['low'] + unit * bars
        if c[i - 1]['close'] <= gann  and  c[i]['close'] > gann:
            return mk(c, i, "long", a, 0.67, "Price broke above Gann 1x1 up angle")
    else:
        bars = len(w) - 1 - hiIdx
        gann = w[hiIdx]['high'] - unit * bars
        if c[i - 1]['close'] >= gann  and  c[i]['close'] < gann:
            return mk(c, i, "short", a, 0.67, "Price broke below Gann 1x1 down angle")
    return make_signal(reason="No Gann angle break")
