...
def anchoredVwapBounce(c):
    if len(c) < 60:
        return make_signal(reason="Insufficient data")
    i = len(c) - 1
    a = atr(c, 14)
    av = anchoredVwap(c, i)
    if c[i]['low'] <= av  and  c[i]['close'] > av  and  c[i]['close'] > c[i]['open']:
        return mk(c, i, "long", a, 0.7, "Bounce off anchored VWAP")
    if c[i]['high'] >= av  and  c[i]['close'] < av  and  c[i]['close'] < c[i]['open']:
        return mk(c, i, "short", a, 0.7, "Rejection at anchored VWAP")
    return make_signal(reason="No anchored-VWAP reaction")
