...
def aroon(c):
    ...
    a = atr(c, 14)
    now = aroonCalc(c, i, p)
    prev = aroonCalc(c, i - 1, p)
    if prev['up'] <= prev.dn  and  now['up'] > now.dn  and  now['up'] > 70:
        return mk(c, i, "long", a, 0.71, "Aroon-Up crossed above Aroon-Down")
    if prev.dn <= prev['up']  and  now.dn > now['up']  and  now.dn > 70:
        return mk(c, i, "short", a, 0.71, "Aroon-Down crossed above Aroon-Up")
    return make_signal(reason=f"Aroon up {now['up']} / dn {now.dn}")
