...
def vwapMagnet(c):
    ...
    vwap = _d['vwap']
    sd = _d['sd']
    if c[i]['close'] < vwap - 2.5 * sd:
        return mk(c, i, "long", a, 0.69, "Far below VWAP — reversion magnet", 1.5)
    if c[i]['close'] > vwap + 2.5 * sd:
        return mk(c, i, "short", a, 0.69, "Far above VWAP — reversion magnet", 1.5)
    return make_signal(reason="Near VWAP")
