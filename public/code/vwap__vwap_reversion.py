...
def vwapReversion(c):
    ...
    vwap = _d['vwap']
    sd = _d['sd']
    if c[i]['low'] <= vwap - 2 * sd  and  c[i]['close'] > c[i]['open']:
        return mk(c, i, "long", a, 0.71, "VWAP -2σ band reversion (long)")
    if c[i]['high'] >= vwap + 2 * sd  and  c[i]['close'] < c[i]['open']:
        return mk(c, i, "short", a, 0.71, "VWAP +2σ band reversion (short)")
    return make_signal(reason="Inside VWAP bands")
