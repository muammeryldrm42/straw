...
def vwapTrend(c):
    ...
    vwap = _d['vwap']
    above = all(x['close'] > vwap for x in c[i - 4: i + 1])
    below = all(x['close'] < vwap for x in c[i - 4: i + 1])
    prevAbove = all(x['close'] > rollingVwap(c, i - 1, 30).vwap for x in c[i - 5: i])
    if above  and  not prevAbove:
        return mk(c, i, "long", a, 0.68, "Trend established above VWAP")
    if below  and  prevAbove:
        return mk(c, i, "short", a, 0.68, "Trend established below VWAP")
    return make_signal(reason="No VWAP trend shift")
