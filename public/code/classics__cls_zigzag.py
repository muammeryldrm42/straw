...
def zigzagBreak(c):
    ...
    a = atr(c, 14)
    z = zigzag(c, i, 0.03)
    if z['dir'] == 1  and  c[i]['close'] > c[z['lastPivotIdx']]['close']  and  c[i - 1]['close'] <= c[z['lastPivotIdx']]['close']:
        return mk(c, i, "long", a, 0.68, "ZigZag confirmed higher swing (uptrend)")
    if z['dir'] == -1  and  c[i]['close'] < c[z['lastPivotIdx']]['close']  and  c[i - 1]['close'] >= c[z['lastPivotIdx']]['close']:
        return mk(c, i, "short", a, 0.68, "ZigZag confirmed lower swing (downtrend)")
    return make_signal(reason="No ZigZag break")
