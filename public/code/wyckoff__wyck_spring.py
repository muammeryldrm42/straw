...
def spring(c):
    ...
    if c[i]['low'] < lo  and  c[i]['close'] > lo  and  c[i]['close'] > c[i]['open']:
        return mk(c, i, "long", c[i]['low'] - a[i] * 0.5, [(lo + hi) / 2, hi], 0.73, "Wyckoff Spring (False break below support, reclaimed)")
    return make_signal(reason="No spring")
