...
def varianceRatio(c):
    ...
    rets1 = []
    rets5 = []
    for k in range(i - 30, (i)+1):
        rets1.append(Math.log(closes[k] / closes[k - 1]))
    for k in range(i - 30, (i)+1, 5):
        if closes[k - 5]:
            rets5.append(Math.log(closes[k] / closes[k - 5]))
    def varOf(arr):
        m = sum(y for y in arr) / len(arr)
        return sum((v - m) ** 2 for v in arr) / len(arr)
    vr = (varOf(rets5) / (5 * varOf(rets1)) if varOf(rets1) else 1)
    slope = (closes[i] - closes[i - 10])
    # VR > 1 = trending (momentum)
    if vr > 1.2  and  slope > 0:
        return mkS(c, i, "long", a, 0.68, f"Variance ratio {vr} (trending up)")
    if vr > 1.2  and  slope < 0:
        return mkS(c, i, "short", a, 0.68, f"Variance ratio {vr} (trending down)")
    return make_signal(reason=f"Variance ratio {vr}")
