...
def vwMacd(c):
    ...
    def vwma(p):
        ...
        for i in range(0, len(c)):
            if i < p - 1:
                out.append(c[i]['close'])
                continue
            pv = 0
            v = 0
            for k in range(0, p):
                pv += c[i - k]['close'] * c[i - k]['volume']
                v += c[i - k]['volume']
            out.append((pv / v if v else c[i]['close']))
        return out
    fast = vwma(12)
    slow = vwma(26)
    macdLine = [v - slow[i] for i, v in enumerate(fast)]
    sig = ema(macdLine, 9)
    i = len(c) - 1
    a = atr(c, 14)
    if macdLine[i - 1] <= sig[i - 1]  and  macdLine[i] > sig[i]:
        return mk(c, i, "long", a, 0.71, "Volume-Weighted MACD bullish cross")
    if macdLine[i - 1] >= sig[i - 1]  and  macdLine[i] < sig[i]:
        return mk(c, i, "short", a, 0.71, "Volume-Weighted MACD bearish cross")
    return make_signal(reason="No VW-MACD cross")
