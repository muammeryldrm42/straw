...
def meanReversionStat(c):
    ...
    zPrev = ((closes[i - 1] - mean) / sd if sd else 0)
    # Aşırı sapmadan ortalamaya dönüş başlangıcı
    if zPrev < -2.5  and  z > zPrev:
        return mkS(c, i, "long", a, 0.7, "Mean reversion from statistical low", 1.5)
    if zPrev > 2.5  and  z < zPrev:
        return mkS(c, i, "short", a, 0.7, "Mean reversion from statistical high", 1.5)
    return make_signal(reason=f"Stat deviation z={z}")
