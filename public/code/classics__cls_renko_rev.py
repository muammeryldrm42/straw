...
def renkoReversal(c):
    ...
    i = len(c) - 1
    a = atr(c, 14)
    _d = renkoDir(c, i, a[i] * 1.5)
    dir = _d['dir']
    prevDir = _d['prevDir']
    if dir == 1  and  prevDir == -1:
        return mk(c, i, "long", a, 0.69, "Renko reversal brick (red→green)")
    if dir == -1  and  prevDir == 1:
        return mk(c, i, "short", a, 0.69, "Renko reversal brick (green→red)")
    return make_signal(reason="No Renko reversal")
