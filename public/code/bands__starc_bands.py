...
def starcBands(c):
    ...
    up = mid[i] + 2 * a[i]
    lo = mid[i] - 2 * a[i]
    if c[i - 1]['low'] <= mid[i - 1] - 2 * a[i - 1]  and  c[i]['close'] > c[i]['open']:
        return mkBreak(c, i, "long", lo - a[i] * 0.5, 0.69, "STARC lower band bounce")
    if c[i - 1]['high'] >= mid[i - 1] + 2 * a[i - 1]  and  c[i]['close'] < c[i]['open']:
        return mkBreak(c, i, "short", up + a[i] * 0.5, 0.69, "STARC upper band rejection")
    return make_signal(reason="Inside STARC bands")
