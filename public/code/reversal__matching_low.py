...
def matchingLow(c):
    ...
    if red(prev)  and  red(cur)  and  abs(prev['close'] - cur['close']) / cur['close'] < 0.003  and  r[i] < 40:
        return mkR(c, i, "long", cur['low'] - 0.5 * a[i], 0.68, "Matching low (support confirmed)")
    return make_signal(reason="No matching low")
