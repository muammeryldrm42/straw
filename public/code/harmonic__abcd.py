...
def abcd(c):
    ...
    cur = c[i]
    if i - D.idx > 5:
        return make_signal(reason="ABCD: pattern too old")
    AB = abs(B.price - A.price)
    BC = abs(C.price - B.price)
    CD = abs(D.price - C.price)
    if AB == 0  or  BC == 0:
        return make_signal(reason="ABCD: degenerate")
    rBC = BC / AB
    rCD = CD / AB
    # AB≈CD (0.8-1.3) ve BC retrace 0.382-0.886
    if rBC < 0.38  or  rBC > 0.89  or  rCD < 0.8  or  rCD > 1.35:
        return make_signal(reason=f"ABCD: ratios off (CD/AB {rCD})")
    bullish = D.type == "L"
    if bullish:
        sl = D.price - 1.5 * a[i]
        r = cur['close'] - sl
        if r <= 0:
            return make_signal(reason="ABCD: invalid risk")
        return make_signal(signal="long", entry=cur['close'], stop_loss=sl, take_profit=[C.price, B.price, A.price], confidence=0.69, reason="ABCD bullish completion")
    sl = D.price + 1.5 * a[i]
    r = sl - cur['close']
    if r <= 0:
        return make_signal(reason="ABCD: invalid risk")
    return make_signal(signal="short", entry=cur['close'], stop_loss=sl, take_profit=[C.price, B.price, A.price], confidence=0.69, reason="ABCD bearish completion")
