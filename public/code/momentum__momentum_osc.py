...
def momentumOsc(c):
    ...
    momArr = [((v - closes[i - p] if i >= p else 0)) for i, v in enumerate(closes)]
    sig = sma(momArr, 9)
    i = len(c) - 1
    a = atr(c, 14)
    if momArr[i - 1] <= sig[i - 1]  and  momArr[i] > sig[i]:
        return mk(c, i, "long", a, 0.68, "Momentum crossed above its signal")
    if momArr[i - 1] >= sig[i - 1]  and  momArr[i] < sig[i]:
        return mk(c, i, "short", a, 0.68, "Momentum crossed below its signal")
    return make_signal(reason="No momentum cross")
