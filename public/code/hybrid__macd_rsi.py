...
def macdRsi(c):
    ...
    i = len(c) - 1
    a = atr(c, 14)
    crossUp = m['macd'][i - 1] <= m['signal'][i - 1]  and  m['macd'][i] > m['signal'][i]
    crossDn = m['macd'][i - 1] >= m['signal'][i - 1]  and  m['macd'][i] < m['signal'][i]
    if crossUp  and  r[i] > 50:
        return mkH(c, i, "long", a, 0.73, "MACD bullish cross + RSI > 50")
    if crossDn  and  r[i] < 50:
        return mkH(c, i, "short", a, 0.73, "MACD bearish cross + RSI < 50")
    return make_signal(reason="No MACD+RSI confluence")
