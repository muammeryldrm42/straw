...
def chikouConfirm(c):
    ...
    chikouNow = c[i]['close']
    pastPrice = c[i - 26]['close']
    pastPricePrev = c[i - 27]['close']
    chikouPrev = c[i - 1]['close']
    if chikouPrev <= pastPricePrev  and  chikouNow > pastPrice:
        return mk(c, i, "long", a, 0.7, "Chikou span crossed above past price")
    if chikouPrev >= pastPricePrev  and  chikouNow < pastPrice:
        return mk(c, i, "short", a, 0.7, "Chikou span crossed below past price")
    return make_signal(reason="No Chikou cross")
