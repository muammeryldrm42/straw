...
def tradersDynamicIndex(c):
    ...
    fast = sma(rsiArr, 2)
    slow = sma(rsiArr, 7)
    if fast[i] > slow[i]  and  fast[i - 1] <= slow[i - 1]  and  rsiArr[i] > 50:
        return mk(c, i, "long", a, 0.7, "TDI fast crossed above slow (bullish)")
    if fast[i] < slow[i]  and  fast[i - 1] >= slow[i - 1]  and  rsiArr[i] < 50:
        return mk(c, i, "short", a, 0.7, "TDI fast crossed below slow (bearish)")
    return make_signal(reason="No TDI cross")
