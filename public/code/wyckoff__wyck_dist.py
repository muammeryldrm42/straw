...
def distribution(c):
    ...
    if tight  and  c[i]['close'] < lo  and  c[i]['volume'] > avgV * 1.3:
        return mk(c, i, "short", hi, [c[i]['close'] - (hi - lo), c[i]['close'] - (hi - lo) * 2], 0.71, "Distribution range breakdown (markdown phase)")
    return make_signal(reason="No distribution breakdown")
