...
def tripleScreen(c):
    ...
    i = len(c) - 1
    a = atr(c, 14)
    trendUp = eLong[i] > eLong[i - 5]
    trendDn = eLong[i] < eLong[i - 5]
    # Screen1: trend, Screen2: oscillator oversold/overbought against trend, Screen3: entry
    if trendUp  and  k[i] < 35  and  m['histogram'][i] > m['histogram'][i - 1]:
        return mkH(c, i, "long", a, 0.74, "Triple Screen: uptrend + oversold pullback + momentum")
    if trendDn  and  k[i] > 65  and  m['histogram'][i] < m['histogram'][i - 1]:
        return mkH(c, i, "short", a, 0.74, "Triple Screen: downtrend + overbought bounce + momentum")
    return make_signal(reason="Triple Screen not aligned")
