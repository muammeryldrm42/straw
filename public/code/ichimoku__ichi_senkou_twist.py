...
def senkouTwist(c):
    ...
    if prev['spanA'] <= prev['spanB']  and  now['spanA'] > now['spanB']  and  c[i]['close'] > now['kijun']:
        return mk(c, i, "long", a, 0.69, "Cloud twist bullish (Span A above Span B)")
    if prev['spanA'] >= prev['spanB']  and  now['spanA'] < now['spanB']  and  c[i]['close'] < now['kijun']:
        return mk(c, i, "short", a, 0.69, "Cloud twist bearish (Span A below Span B)")
    return make_signal(reason="No cloud twist")
