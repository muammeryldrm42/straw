...
def fibConfluence(c):
    ...
    a = atr(c, 14)
    s = swing(c, i)
    e = ema([x['close'] for x in c], 50)
    if s.range <= 0:
        return make_signal(reason="No swing")
    if s['up']:
        f618 = s['hi'] - s.range * 0.618
        if abs(f618 - e[i]) < a[i] * 1.5  and  c[i]['low'] <= f618 + a[i]  and  c[i]['close'] > c[i]['open']:
            return mk(c, i, "long", f618 - a[i] * 1.5, [c[i]['close'] + s.range * 0.3, s['hi']], 0.74, "Fib 0.618 + EMA50 confluence (long)")
    else:
        f618 = s['lo'] + s.range * 0.618
        if abs(f618 - e[i]) < a[i] * 1.5  and  c[i]['high'] >= f618 - a[i]  and  c[i]['close'] < c[i]['open']:
            return mk(c, i, "short", f618 + a[i] * 1.5, [c[i]['close'] - s.range * 0.3, s['lo']], 0.74, "Fib 0.618 + EMA50 confluence (short)")
    return make_signal(reason="No fib+EMA confluence")
