...
def mcginleyDynamic(c):
    if len(c) < 30:
        return make_signal(reason="Insufficient data")
    i = len(c) - 1
    a = atr(c, 14)
    md = mcginley(c, 14)
    if c[i]['close'] > md[i]  and  c[i - 1]['close'] <= md[i - 1]:
        return mk(c, i, "long", a, 0.69, "Price crossed above McGinley Dynamic")
    if c[i]['close'] < md[i]  and  c[i - 1]['close'] >= md[i - 1]:
        return mk(c, i, "short", a, 0.69, "Price crossed below McGinley Dynamic")
    return make_signal(reason="No McGinley cross")
