...
def murreyMath(c):
    ...
    rng = hi - lo
    if rng <= 0:
        return make_signal(reason="No range")
    lv = lambda n: lo + (rng * n) / 8
    # 0/8 ve 8/8 ekstrem; 2/8 ve 6/8 dönüş bölgeleri; 4/8 ana destek/direnç
    if c[i]['low'] <= lv(2)  and  c[i]['close'] > lv(2)  and  c[i]['close'] > c[i]['open']:
        return mk(c, i, "long", a, 0.68, "Murrey 2/8 bounce (oversold pivot)")
    if c[i]['high'] >= lv(6)  and  c[i]['close'] < lv(6)  and  c[i]['close'] < c[i]['open']:
        return mk(c, i, "short", a, 0.68, "Murrey 6/8 rejection (overbought pivot)")
    return make_signal(reason="Between Murrey levels")
