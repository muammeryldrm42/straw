...
// Gerçek Fear & Greed — alternative.me (ücretsiz, key yok). Başarısızsa BTC momentum proxy'sine düşer.
async function fetchFearGreed(): Promise<number | null> {
  try {
    const r = await fetch("https://api.alternative.me/fng/?limit=1", {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; StrategyDEX/1.0)", Accept: "application/json" },
      cache: "no-store",
    });
    if (!r.ok) return null;
...
export async function GET(req: Request) {
  ...
  // altseason proxy: BTC'yi 7d'de geçen coin oranı
  const beatBtc = coins.filter((c) => c.symbol !== "BTC" && c.pctChange7d > btc7).length;
  const altseason = clamp(Math.round((beatBtc / Math.max(1, coins.length - 1)) * 100), 0, 100);
  // fear&greed: gerçek (alternative.me) → yoksa BTC momentum proxy
  const fgReal = await fetchFearGreed();
  const fearGreed = fgReal ?? clamp(Math.round(50 + btc7 * 1.5 + avg24 * 1.2), 5, 95);
  ...
  return NextResponse.json({ decision, skills, breadth: globals.breadth, scanned: coins.length });
}
