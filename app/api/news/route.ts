...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let url: string;
    if (mode === "hot") {
      url = `https://openapi.sosovalue.com/openapi/v1/news/hot?page=${page}&page_size=100&language=${apiLang}`;
    } else {
      const catParam = cats ? `&category=${cats}` : "";
      url = `https://openapi.sosovalue.com/openapi/v1/news?page=${page}&page_size=100&language=${apiLang}${catParam}`;
    }
    ...
    // Sentiment her zaman İngilizce başlıktan hesaplanır (dil değişse de etiket aynı kalsın).
    // Seçili dil İngilizce değilse, İngilizce başlıkları ayrı çekip id ile eşleştir.
    const enTitle: Record<string, string> = {};
    if (apiLang !== "en") {
      try {
        const enUrl = url.replace(`language=${apiLang}`, "language=en");
        const enRes = await fetch(enUrl, { headers: { "x-soso-api-key": key, Accept: "application/json" }, next: { revalidate: 60 } });
        if (enRes.ok) {
          const enJson = await enRes.json();
          (enJson?.data?.list || []).forEach((n: any) => { enTitle[String(n.id)] = n.title || ""; });
        }
    ...
    const items = list.map((n) => {
      ...
      const tForSent = apiLang === "en" ? (n.title || "") : (enTitle[id] || n.title || "");
      return {
        id,
        sourceLink: n.source_link || n.original_link || "",
        releaseTime: n.release_time || n.create_time || 0,
        ...
        currencies: Array.from(new Set(((n.matched_currencies || n.matchedCurrencies || n.matchedCurrency || []) as any[]).map((c: any) => pickCur(c, apiLang)).filter(isValidTicker))),
        title: n.title || "",
        content: n.content || "",
        sentiment: newsSentiment(tForSent),
      };
