...
interface Payload { decision: Decision; skills: SkillOut[]; breadth: Breadth; scanned: number; fgIsReal?: boolean; }
...
export default function SkillsSignalPage() {
  ...
  useEffect(() => {
    (async () => {
      // Gerçek F&G'yi tarayıcıdan dene (CMC keyless public). Olmazsa server tarafı CMC/alternative.me dener.
      let fg = "";
      try {
        const fr = await fetch("https://pro-api.coinmarketcap.com/trial-pro-api/v3/fear-and-greed/latest", { cache: "no-store" });
        if (fr.ok) {
          const fd = await fr.json();
          const v = parseInt(fd?.data?.value ?? fd?.data?.[0]?.value, 10);
          if (isFinite(v)) fg = String(v);
        }
      } catch { /* CORS/erişim olmazsa server tarafı dener */ }
      try {
        const r = await fetch(`/api/skills-signal${fg ? `?fg=${fg}` : ""}`);
        const d = await r.json();
        if (d.decision) setData(d); else setErr(true);
      } catch {
        setErr(true);
      } finally {
        setLoading(false);
        ...
        <>
          <div style={{ border: `1px solid ${regimeColor(m.regime)}`, borderRadius: 12, padding: "22px 24px", marginBottom: 22, background: "var(--bg-soft)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10 }}>
              <Metric label={t("skills.m_fg")} value={data!.fgIsReal === false ? "n/a" : `${m.fearGreed} · ${fgLabel(m.fearGreed)}`} color={data!.fgIsReal === false ? "var(--text-faint)" : m.fearGreed >= 55 ? "var(--green)" : m.fearGreed <= 45 ? "var(--red)" : "#f5a623"} />
