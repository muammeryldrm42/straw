...
import { downloadPnlCard, pnlCardDataURL, type PnlCardData } from "@/lib/pnlCard";
...
function DemoInner() {
  ...
  return (
    <main className="container" style={{ paddingTop: 24, paddingBottom: 48 }}>
            ...
            <Field label={t("demo.strategy")}>
              <div style={{ position: "relative" }}>
                <button
                  ...
                  style={{ ...selStyle, textAlign: "left", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}
                >
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {stratId === "__manual__" ? t("demo.no_strategy") : (getStrategy(stratId)?.name ?? t("demo.no_strategy"))}
                  </span>
                  <span style={{ opacity: 0.5, fontSize: 11 }}>▾</span>
                </button>
                {stratOpen && (
                  <>
                    <div onClick={() => { setStratOpen(false); setStratQuery(""); }} style={{ position: "fixed", inset: 0, zIndex: 40 }} />
                    <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 50, background: "var(--bg-soft)", border: "1px solid var(--border-glow)", borderRadius: 8, maxHeight: 320, overflowY: "auto", boxShadow: "0 14px 36px rgba(0,0,0,0.45)" }}>
                      <input
                        type="text"
                        autoFocus
                        value={stratQuery}
                        onChange={(e) => setStratQuery(e.target.value)}
                        placeholder={t("demo.search_strategy")}
                        style={{ width: "100%", boxSizing: "border-box", padding: "9px 12px", background: "var(--bg-soft)", border: "none", borderBottom: "1px solid var(--border)", color: "var(--text)", fontSize: 13, position: "sticky", top: 0, outline: "none" }}
                      />
                      <div
                        onClick={() => { setStratId("__manual__"); setLastSignal(null); setStratOpen(false); setStratQuery(""); }}
                        style={{ padding: "9px 12px", cursor: "pointer", fontSize: 13, borderBottom: "1px solid var(--border)", color: "var(--text)", background: stratId === "__manual__" ? "var(--border-glow)" : "transparent" }}
                      >
                        {t("demo.no_strategy")}
                      </div>
                      {ALL_CATEGORIES.map((cat) => {
                        const q = stratQuery.trim().toLowerCase();
                        const matches = STRATEGIES.filter((s) => s.category === cat && (!q || s.name.toLowerCase().includes(q)));
                        if (matches.length === 0) return null;
                        return (
                          <div key={cat}>
                            <div style={{ padding: "6px 12px", fontSize: 9, textTransform: "uppercase", letterSpacing: 0.5, color: "var(--text-faint)", background: "rgba(255,255,255,0.03)" }}>{t(`cat.${cat}`)}</div>
                            {matches.map((s) => (
                              <div
                                key={s.id}
                                onClick={() => { if (!s.run) return; setStratId(s.id); setLastSignal(null); setStratOpen(false); setStratQuery(""); }}
                                style={{ padding: "8px 12px 8px 18px", cursor: s.run ? "pointer" : "not-allowed", fontSize: 13, opacity: s.run ? 1 : 0.4, color: "var(--text)", background: stratId === s.id ? "var(--border-glow)" : "transparent" }}
                              >
                  ...
                  {wallet.positions.map((p) => {
                    ...
                    return (
                      <tr key={p.id} style={{ borderTop: "1px solid var(--border)" }}>
                        ...
                        <td style={td}>
                          <button className="btn btn-ghost" onClick={() => closePos(p.id)} style={{ padding: "4px 10px", fontSize: 11 }}>{t("tbl.close")}</button>
                          <button className="btn btn-ghost" title="PnL card" disabled={!known} onClick={() => setPnlCard({ symbol: sym(p.symbol), side: p.side, leverage: p.leverage, entry: p.entry, current: px, pnl, roe, margin: p.margin, size: p.size, strategy: p.strategy })} style={{ padding: "4px 9px", fontSize: 11, marginLeft: 4, opacity: known ? 1 : 0.4 }}>⤓</button>
                        </td>
                      </tr>
                    );
                  })}
                ...
                {wallet.history.map((tr) => (
                  <div key={tr.id + tr.closeTime} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--border)", fontSize: 12 }}>
                    ...
                    <div style={{ textAlign: "right", display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <button className="btn btn-ghost" title="PnL card" onClick={() => setPnlCard({ symbol: sym(tr.symbol), side: tr.side, leverage: tr.leverage, entry: tr.entry, current: tr.exit, pnl: tr.pnl, roe: tr.roe, margin: tr.margin, size: tr.size, strategy: tr.strategy, closed: true })} style={{ padding: "3px 8px", fontSize: 11 }}>⤓</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {pnlCard && (
        <div onClick={() => setPnlCard(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 440, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={pnlCardDataURL(pnlCard)} alt="PnL card" style={{ width: "100%", borderRadius: 18, border: "1px solid var(--border)", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }} />
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => downloadPnlCard(pnlCard)} className="btn" style={{ padding: "11px 28px", fontWeight: 700, fontSize: 14 }}>{t("demo.download") || "Download"}</button>
              <button onClick={() => setPnlCard(null)} className="btn btn-ghost" style={{ padding: "11px 22px", fontSize: 14 }}>{t("common.close") || "Close"}</button>
            </div>
          </div>
        </div>
      )}
