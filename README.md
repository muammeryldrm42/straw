...
A chat terminal wired to the site's real engines. Type a command, get one readable answer:
- **`hunt`** : scans the most active markets through **all 270+ strategies** and ranks the strongest LONG/SHORT setups, with one-tap buttons to open the best one (`hunt long` / `hunt short` for directional)
- **`market`** : one-message overview — regime, Fear & Greed, BTC dominance, breadth, **BTC/ETH ETF flows**, risk flags and the playbook bias
- **`btc`** : just type a ticker — full strategy scan + ETF flows in one shot; `why btc` gives a reasoned verdict across signals+funding+F&G+ETF; `setup btc` builds an ATR-sized trade plan; `confluence btc` shows which strategy *families* agree; `compare btc eth` goes head-to-head; `btc 4h` scans any timeframe (1m–1d)
- **`movers` / `news` / `fg` / `ssi` / `funding btc`** : 24h movers, SoSoValue news with sentiment tone, Fear & Greed with weekly trend, SSI index momentum, perp funding crowd read
- **Trade from chat** : `btc long 100 5x sl 2 tp 5` opens a real demo position (same wallet as Demo Trade); `close btc`, `closeall`, `positions`, `pnl`, `risk` manage it
- **No LLM** : a deterministic command engine running the same open-source strategies — no hallucinated numbers, honest empty states, session-persistent chat

Full command reference: **[AI-TRADE.md](./AI-TRADE.md)**

### 🤖 Telegram bot — the hunter on Telegram
The same market-intelligence and coin-analysis commands, delivered on Telegram: **[t.me/STRATEGYDEX_bot](https://t.me/STRATEGYDEX_bot)**.
- **Market intel** : `/hunt`, `/market`, `/movers`, `/news`, `/fg`, `/funding <coin>`
- **Coin analysis** : `/coin btc` (full scan + ETF), `/signals`, `/etf`, `/why`, `/setup`, `/confluence`, `/compare`, `/price` — bare tickers work too (just send `btc`)
- A **thin client**: a webhook (`/api/telegram/webhook`) that calls the site's own APIs plus public data (funding, Fear & Greed) and formats the result for chat. Event-driven, runs on a free plan with no cron. Info only — no trading is exposed on Telegram.

Full command reference and setup guide: **[TELEGRAM.md](./TELEGRAM.md)**
...
### 🔌 MCP Server — use the engine from any AI assistant
Strategy DEX exposes its strategy engine as an **MCP (Model Context Protocol) server** at `/api/mcp`, so AI assistants can call it directly:
- **list_categories** / **list_strategies** — browse the 270+ strategies by category
- **scan_live_signals** — run every strategy on a symbol and get what's firing long vs short
- **hunt_setups** — scan active markets for the strongest long/short setups
- **analyze_coin** — reasoned verdict fusing signals, families, funding, F&G and ETF
- **get_market_overview** — regime, Fear & Greed, dominance, ETF flows, risk flags
- **get_funding / get_fear_greed / get_news / get_movers / get_ssi** — live market data
- **get_trade_setup** — ATR-sized trade plan; **backtest_strategy** — backtest on real history; **get_etf_flows** — SoSoValue ETF flows

Full tool reference and connection guide: **[MCP.md](./MCP.md)**

JSON-RPC 2.0 over Streamable HTTP, CORS-enabled, no SDK required. This turns the whole library into AI-callable tools.
